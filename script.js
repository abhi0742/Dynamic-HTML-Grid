...
productList.forEach(function (value, index, list) {
    ...
    imgcard.src = value.preview;

    parentcard.appendChild(imgcard);
    ...
    description.innerHTML = value.brand;
    ...
    parentcard.appendChild(cardinfo);


    const prductbox = document.getElementById("product-box");
    const clothbox = document.getElementById("cloth-box");

    if (value.isAccessory) {
        prductbox.appendChild(parentcard)
    } else {
        clothbox.appendChild(parentcard)
    }
})